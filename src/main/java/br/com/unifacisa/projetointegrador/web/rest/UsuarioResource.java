...
import br.com.unifacisa.projetointegrador.repository.UsuarioRepository;
...
/**
 * REST controller for managing {@link br.com.unifacisa.projetointegrador.domain.Usuario}.
 */
@RestController
...
public class UsuarioResource {
        ...
        Optional<Usuario> result = usuarioRepository
            ...
            .map(existingUsuario -> {
                ...
                if (usuario.getSenhaHash() != null) {
                    existingUsuario.setSenhaHash(usuario.getSenhaHash());
                }
     ...
     */
    @GetMapping("")
    public List<Usuario> getAllUsuarios(@RequestParam(name = "filter", required = false) String filter) {
        if ("funcionario-is-null".equals(filter)) {
            LOG.debug("REST request to get all Usuarios where funcionario is null");
            return StreamSupport.stream(usuarioRepository.findAll().spliterator(), false)
                .filter(usuario -> usuario.getFuncionario() == null)
                .toList();
        }
